...
public class SearchBookCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (allBooks == null) {
			...
			commandHelper.putAttributeInSession(request, currentPageNumber, numberOfAllPages,
					CommandHelper.SEARCH_BOOK, value);
		}
		
		String url = commandHelper.takeURL(request);
		System.out.println(url + " url in searchcommand before");
		request.getSession().setAttribute(CommandHelper.URL, url);
		System.out.println(request.getSession().getAttribute(CommandHelper.URL) + " url in searchcommand after");
